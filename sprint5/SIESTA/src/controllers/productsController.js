...
const productsController = {
    ...
    search: (req, res) => {
		const resultSearch = req.query.keywords.toLowerCase();
		const productsResult = products.filter(product => product.name.toLowerCase().includes(resultSearch));
		res.render('results', {productsResult});
	},
    ...
    editProduct: (req, res) => {
        ...
        const {name, price, description, colors, size1, size2, size3, size4, genre} = req.body;
    ...
    menProducts: (req, res) => {
        const productsForMen = products.filter(product => product.genre == 'men');
        res.render('menProducts', {productsForMen});
    },

    womenProducts: (req, res) => {
        const productsForWomen = products.filter(product => product.genre == 'women');
